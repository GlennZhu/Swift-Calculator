...
class AxesDrawer
{
    ...
    func drawInRect(bounds: CGRect, origin: CGPoint, pointsPerUnit: CGFloat, functionToDraw: (Double -> Double)?)
    {
        ...
        var previouxPixelValid = false
        for pixelX in Int(bounds.minX)...Int(bounds.maxX) {
            let coorX = (Double(pixelX) - Double(origin.x)) / Double(pointsPerUnit)
            if let coorY = functionToDraw?(coorX) {
                if coorY.isNormal || coorY.isZero {
                    let pixelY = CGFloat(Double(origin.y) - Double(coorY) * Double(pointsPerUnit))
                    let newPixelPoint = CGPoint(x: align(CGFloat(pixelX)), y: align(CGFloat(pixelY)))
                    if !previouxPixelValid {
                        path.moveToPoint(newPixelPoint)
                    }
                    path.addLineToPoint(newPixelPoint)
                    previouxPixelValid = true
                }
            } else {
                previouxPixelValid = false
            }
        }
        path.stroke()
